...
with app.app_context():
        ...
        db.session.add_all(
            [
                Product(
                    ...
                    details="Unlimited API calls, dedicated support",
                    catalogue_info="SKU: AI-ENT-001",
                ),
                Product(
                    name="Startup Starter Pack",
                    description="Essential tools for small teams",
                    details="Basic AI features, email support",
                    catalogue_info="SKU: ST-BAS-101",
                ),
                Product(
                    name="Consulting Services",
                    description="Expert implementation help",
                    details="Hourly rate, onsite available",
                    catalogue_info="SKU: SRV-CON-999",
                ),
            ]
        )
        db.session.commit()
    ...
    if not Feedback.query.first():
        ...
        if sales_user:
            ...
            lead_samples = [
                (
                    "Loved the demo, budget approved, wants to start next week.",
                    0.95,
                    "High",
                ),
                (
                    "Very keen! Asked for a custom quote for 500 seats. Hot lead!",
                    0.98,
                    "High",
                ),
                (
                    "Meeting went okay. They liked Feature A. Need to nurture.",
                    0.55,
                    "Medium",
                ),
                (
                    "Just looking around, no immediate need. Maybe next year.",
                    0.20,
                    "Low",
                ),
                (
                    "Stuck in an existing contract for 6 months. Call back later.",
                    0.30,
                    "Low",
                ),
                (
                    "Great conversation. Decision maker needs approval from CEO.",
                    0.75,
                    "High",
                ),
                (
                    "Standard inquiry, sent pricing sheet. Waiting to hear back.",
                    0.50,
                    "Medium",
                ),
                (
                    "Had technical issues during the demo, they got frustrated.",
                    0.15,
                    "Low",
                ),
                (
                    "Impressed by the AI features. Wants a follow-up with their CTO.",
                    0.92,
                    "High",
                ),
                (
                    "Their team is too small for the Enterprise plan, pitched Startup pack.",
                    0.45,
                    "Medium",
                ),
            ]
            ...
            feedback_samples = [
                (
                    "I am extremely happy with the support team, solved my issue in 5 minutes!",
                    0.9,
                    "Positive",
                ),
                (
                    "The new update is fantastic, everything runs so much faster.",
                    0.8,
                    "Positive",
                ),
                (
                    "It's an okay product, but it's missing a few key features.",
                    0.1,
                    "Neutral",
                ),
                (
                    "I am so frustrated. The app crashed and I lost all my work.",
                    -0.8,
                    "Negative",
                ),
                ...
                (
                    "I like the product, it does exactly what it says it will do.",
                    0.6,
                    "Positive",
                ),
                ...
                (
                    "Just wanted to say thanks, this tool saved me hours of work.",
                    1.0,
                    "Positive",
                ),
            ]
...
def login():
    ...
    try:
        db.session.add(
            ActivityLog(
                user_id=user.id, action="login", details=f"User logged in as {role}"
            )
        )
        db.session.commit()
    except Exception:
    ...
    try:
        db.session.add(
            ActivityLog(
                user_id=new_user.id,
                action="register",
                details=f"New user self-registered: {username} ({role})",
            )
        )
        db.session.commit()
    except Exception:
...
def submit_lead(current_user):
    ...
    db.session.add(
        ActivityLog(
            ...
            action="lead_submit",
            details=f"Lead {new_entry.id} submitted (Score: {lead_score})",
        )
    )
    db.session.commit()
...
def analyze_feedback(current_user):
    ...
    if zero_shot_pipeline:
        ...
        sentiment_score = predict_probability(text)
    ...
    db.session.add(
        ActivityLog(
            ...
            details=f"Feedback {new_entry.id} submitted (Sentiment: {sentiment_label})",
        )
    )
    db.session.commit()
...
def get_dashboard(current_user):
    ...
    high_quality_texts = [
        f.text for f in Feedback.query.filter(Feedback.lead_label == "High").all()
    ]
    source_text = (
        " ".join(high_quality_texts)
        if len(high_quality_texts) > 5
        else " ".join(
            [f.text for f in Feedback.query.filter(Feedback.lead_label is not None).all()]
        )
    )
    ...
    for f in Feedback.query.order_by(Feedback.timestamp.desc()).all():
        w.writerow(
            [
                ...
                f.lead_label or "N/A",
                f"{f.sentiment_score:.2f}" if f.sentiment_score is not None else "N/A",
                f.sentiment_label or "N/A",
            ]
        )
...
def add_user(current_user):
    ...
    db.session.add(
        ActivityLog(
            user_id=current_user.id,
            action="user_add",
            details=f"Added user: {user.username} ({user.role})",
        )
    )
    db.session.commit()
        ...
        jsonify(
            {
                ...
                "version": "1.0",
                "ml_model": "loaded" if zero_shot_pipeline else "not_loaded",
