...
def seed_database():
    with app.app_context():
        ...
        # =======================================================
        # 1️⃣ Seed Default Users (only if missing)
        # =======================================================
        if not User.query.first():
            ...
            sales = User(username='sales', role='salesperson'); sales.set_password('sales123')
            db.session.add_all([dev, manager, sales])
            db.session.commit()
        ...
        # =======================================================
        # 2️⃣ Seed Sample Products (only if missing)
        # =======================================================
        if not Product.query.first():
            ...
            products = [
                Product(
                    ...
                    details='Features: Durable, Eco-friendly',
                    catalogue_info='SKU: PA-001'
                ),
                Product(
                    name='Product B',
                    description='Premium professional solution',
                    details='Features: Advanced, Warranty included',
                    catalogue_info='SKU: PB-002'
                ),
                Product(
                    name='Product C',
                    description='Budget-friendly starter kit',
                    details='Features: Compact, Great value',
                    catalogue_info='SKU: PC-003'
                )
            ]
        ...
        # =======================================================
        # 3️⃣ Seed Feedback Data (only if missing)
        # =======================================================
        if not Feedback.query.first():
            print("💬 No feedback data found — seeding dashboard entries...")

            sales_user = User.query.filter_by(role='salesperson').first()
            if not sales_user:
                print("❌ Error: No salesperson found! Cannot assign feedback.")
                return
            ...
            for _ in range(2):  # Duplicate loop to make ~20 feedback entries
            ...
            db.session.commit()
            print("✅ Feedback seeded successfully! Dashboard data ready.")
        else:
            print("📊 Feedback entries already exist — skipping seeding.")

        # =======================================================
        # ✅ Final Summary
        # =======================================================
        print("\n🎉 Seeding completed successfully!")
        print(f"👥 Users: {User.query.count()} | 📦 Products: {Product.query.count()} | 💬 Feedback: {Feedback.query.count()}")
